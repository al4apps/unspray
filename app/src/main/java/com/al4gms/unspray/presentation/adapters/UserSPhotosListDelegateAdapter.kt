...
import com.al4gms.unspray.databinding.ItemUserPhotoBinding
...
class UserSPhotosListDelegateAdapter(
    private val onItemClick: (contentId: String, view: View) -> Unit,
) : AbsListItemAdapterDelegate<Content.Photo, Content, UserSPhotosListDelegateAdapter.ViewHolder>() {
    ...
    override fun onCreateViewHolder(parent: ViewGroup): ViewHolder {
        return ViewHolder(parent.inflate(R.layout.item_user_photo), onItemClick)
    }
    ...
    class ViewHolder(
        ...
        private val binding = ItemUserPhotoBinding.bind(view)
        private var photoId: String? = null
        private val downloader = DownloaderImpl(itemView.context)
        private var urlToDownload: String? = null

        init {
            view.setOnClickListener { photoId?.let { onItemClick(it, itemView) } }
            binding.downloadTextView.setOnClickListener { downloadPhoto() }
            binding.downloadImageView.setOnClickListener { downloadPhoto() }
        }

        private fun downloadPhoto() {
            urlToDownload?.let { url ->
                if (downloader.downloadFileWithDM(url) != -1L) {
                    Snackbar.make(itemView, R.string.download_started_toast, Snackbar.LENGTH_SHORT).show()
                }
            }
        }
        ...
        fun bind(photo: Content.Photo) {
            itemView.transitionName =
                itemView.resources.getString(R.string.photo_item_transition_name, photo.id)
            photoId = photo.id
            urlToDownload = photo.urls.raw
            ...
            binding.usernameTextView.text =
                itemView.resources.getString(R.string.at_mail_with_text, photo.user.username)
